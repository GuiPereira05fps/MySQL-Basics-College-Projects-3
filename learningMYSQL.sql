...
idFinalizacao INT PRIMARY KEY AUTO_INCREMENT,
...
CONSTRAINT fk_idTecnico
	FOREIGN KEY (idTecnico)
    REFERENCES TECNICO (idTecnico),
    
CONSTRAINT fk_finalizacao
	FOREIGN KEY (idFinalizacao)
    REFERENCES FINALIZACAO (idFinalizacao)
);
...
CONSTRAINT fk_ordem
	FOREIGN KEY (idOrdem)
    REFERENCES ORDEMSERVICO (idOrdem),
...
INNER JOIN Servico S ON S.idServico = OSS.idServico
ORDER BY idOrdem ASC;

SELECT OSS.idOrdem,
SUM(OSS.quantidade * OSS.valorUnitario) as valorTotal
FROM OrdemServico_Servico OSS
